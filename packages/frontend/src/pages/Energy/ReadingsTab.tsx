...
import type { CreateEnergyReadingInput, EnergyReadingDto, EnergyUnit, MeterType } from '@life-manager/shared';
  ...
  if (!filtered || filtered.length === 0) {
    return (
      <p className="text-sm text-slate-500">
        {selectedYear === 'all' ? 'No readings yet — add one above.' : `No readings in ${selectedYear}.`}
      </p>
    );
  }

  const sorted = [...filtered].sort((a, b) => b.readingDate.localeCompare(a.readingDate));
...
export function ReadingsTab({ selectedYear }: { selectedYear: YearFilterValue }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="card-surface p-4">
        ...
        <ReadingsList selectedYear={selectedYear} />
      </section>
    </div>
  );
}
