...
export function MetricTile({ label, value, tone }: { label: string; value: number; tone: 'in' | 'out' | 'net' }) {
  const colorClass =
...
export function groupByMonth(days: { date: string; moneyIn: number; moneyOut: number; net: number }[]): MonthlyBucket[] {
...
export function HomeContent({ selectedYear }: { selectedYear: YearFilterValue }) {
